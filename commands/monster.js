            ...
            function battle(MAXATK_PLAYER, MAXATK_MONSTER, HEALTH_PLAYER, HEALTH_MONSTER, DODGEPLAYER, CRITPLAYER){
                ...
                while(HEALTH_PLAYER != 0 || HEALTH_MONSTER != 0){
                    ...
                    if(CRITPLAYER == false){
                        ...
                        // Reflect Ultimate
                        if((Math.floor(Math.random() * 100) + 1) < playerStats.player.ultimate.reflect){
                            ULTIMATE = 'You use your Ultimate: :mirror: `Reflect`'
                        };
                        if((Math.floor(Math.random() * 100) + 1) < playerStats.player.ultimate.heal){
                            ULTIMATE = 'You use your Ultimate: :four_leaf_clover: `Heal`'
                        };
                        if((Math.floor(Math.random() * 100) + 1) < playerStats.player.ultimate.luckyStrike){
                            ULTIMATE = 'You use your Ultimate: :mending_heart: `Lucky Strike`'
                        }
                    ...
                    if (HEALTH_PLAYER <= 0){
                        ...
                        var losexp = Math.floor((balance.eco.xp*10)/100)

                        balance.eco.coins = Math.floor(balance.eco.coins - losecoin)
                        balance.eco.xp = Math.floor(balance.eco.xp - losexp)
                        balance.save()
                        ...
                        var battleEmbed = new Discord.MessageEmbed()
                            ...
                            .addFields(
                            { name: '**📊 STATS :**\n', value: `You attacked **${NB_ATTACK_PLAYER}x** and did **${ATK_SOMME_PLAYER}** damage to the Monster\nThe Monster attacked **${NB_ATTACK_MONSTER}x** and did **${ATK_SOMME_MONSTER}** damage to you\n:boxing_glove: You dodged **${NB_DODGE}x** the attacks of the monster, and put **${NB_CRIT}** critical hits!\n\nYou Lose !\n${ULTIMATE}\n${`🎁`} You lose **10%** of your :izakaya_lantern: (**${losexp}**) and **10%** of your :coin: (**${losecoin}**)...`, inline: true },
                            )
                    ...
                    if (HEALTH_MONSTER <= 0){
                        ...
                        var battleEmbed = new Discord.MessageEmbed()
                            ...
                            .setDescription(`**:crossed_swords: BATTLE**\n${client.users.cache.get(user.id).username} :vs: Monster\n**${`:tada: YOU'VE WON!**`}`)
                            .addFields(
                            { name: '**📊 STATS :**\n', value: `You attacked **${NB_ATTACK_PLAYER}x** and did **${ATK_SOMME_PLAYER}** damage to the Monster\nThe Monster attacked **${NB_ATTACK_MONSTER}x** and did **${ATK_SOMME_MONSTER}** damage to you\n:boxing_glove: You dodged **${NB_DODGE}x** the attacks of the monster, and put **${NB_CRIT}** critical hits!\n\nYou Win !\n${`🎁`} And get: **${randomxp}** :izakaya_lantern: and **${randomcoin}** :coin:`, inline: true },
